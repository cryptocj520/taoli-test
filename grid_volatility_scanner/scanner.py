...
class GridVolatilityScanner:
            ...
            try:
                ...
                if should_reconnect and not self._is_reconnecting:
                    ...
                    if self._reconnect_count >= self._max_reconnect_attempts:
                        logger.error(
                            f"❌ 已达到最大重连次数 ({self._max_reconnect_attempts})，停止自动重连"
                        )
                    else:
                        # 触发重连
    ...
    def _log_connection_health(self, current_time: datetime):
        ...
        logger.info(f"  重连次数: {self._reconnect_count}/{self._max_reconnect_attempts}")
        logger.info("=" * 60)
    ...
    async def _reconnect_websocket(self):
        ...
        try:
            logger.warning(f"🔄 开始重连 WebSocket (第 {self._reconnect_count} 次)...")
            ...
            try:
                ...
                # 重新创建虚拟网格（这会重新订阅）
                await self._create_virtual_grids(symbols_to_resubscribe)
                ...
                logger.info(f"✅ WebSocket重连完成 (第 {self._reconnect_count} 次)")
