...
class LighterWebSocket(LighterBase):
    ...
    async def subscribe_orderbook(self, market_index_or_symbol, callback: Optional[Callable] = None):
        """
        订阅订单簿（使用直接WebSocket，而不是SDK的WsClient）

        Args:
            market_index_or_symbol: 市场索引或交易对符号
            callback: 订单簿回调函数（可选，None表示使用统一回调）
        """
        print(f"🔍 [Lighter WebSocket] subscribe_orderbook 被调用: market_index_or_symbol={market_index_or_symbol}, callback={callback is not None}")
        ...
        if callback is not None:
            ...
            if callback not in self._orderbook_callbacks:
                ...
                print(f"✅ [Lighter WebSocket] 已注册订单簿回调函数（总数: {len(self._orderbook_callbacks)}）")
                logger.info(f"✅ 已注册订单簿回调函数（总数: {len(self._orderbook_callbacks)}）")
            else:
                print(f"⚠️ [Lighter WebSocket] 回调函数已在列表中，跳过注册")
        
        if isinstance(market_index_or_symbol, str):
            symbol = market_index_or_symbol
            market_index = self.get_market_index(symbol)
            if market_index is None:
                print(f"❌ [Lighter WebSocket] 未找到交易对 {symbol} 的市场索引")
                logger.warning(f"未找到交易对 {symbol} 的市场索引")
                return
            print(f"🔍 [Lighter WebSocket] 找到市场索引: {symbol} -> market_index={market_index}")
        else:
            market_index = market_index_or_symbol
            symbol = self._get_symbol_from_market_index(market_index)
            print(f"🔍 [Lighter WebSocket] 使用市场索引: market_index={market_index} -> symbol={symbol}")
        ...
        if market_index not in self._subscribed_markets:
            ...
            print(f"📡 [Lighter WebSocket] 已订阅订单簿: {symbol} (market_index={market_index}), 订阅列表: {self._subscribed_markets}")
            logger.info(f"📡 已订阅订单簿: {symbol} (market_index={market_index})")

            # 🔥 确保直接WebSocket连接已启动，然后发送订阅
            await self._ensure_direct_ws_running()
        else:
            print(f"ℹ️ [Lighter WebSocket] market_index={market_index} 已在订阅列表中，跳过")
    ...
    async def _ensure_direct_ws_running(self):
        ...
        if self._direct_ws_task and not self._direct_ws_task.done():
            ...
            # 启动新任务
            logger.info(
                f"🚀 启动新WebSocket任务（待订阅: {len(self._subscribed_market_stats)} 个market_stats, {len(self._subscribed_markets)} 个orderbook）")
        ...
        for i in range(0, total_count, batch_size):
            ...
            for market_index in batch:
                ...
                try:
                    ...
                    # 🔥 统计发送的字节数
                    self._network_bytes_sent += len(msg_str.encode('utf-8'))
                    await self._direct_ws.send(msg_str)
                    sent_count += 1
    ...
    async def _send_orderbook_subscriptions(self):
        ...
        logger.debug(f"准备发送 {total_count} 个订单簿订阅")
        ...
        for i in range(0, total_count, batch_size):
            batch = self._subscribed_markets[i:i + batch_size]
            
            for market_index in batch:
                subscribe_msg = {
                    "type": "subscribe",
                    "channel": f"order_book/{market_index}"  # 🔥 参考 test_sol_orderbook.py 的格式
                }
                try:
                    msg_str = json.dumps(subscribe_msg)
                    # 🔥 统计发送的字节数
    ...
    async def _recreate_ws_client(self):
        ...
        try:
            ...
            print(f"🔍 [Lighter WebSocket] 创建WsClient: host={ws_host}, order_book_ids={self._subscribed_markets}, account_ids={self._subscribed_accounts}")
            print(f"🔍 [Lighter WebSocket] 回调函数: on_order_book_update={self._on_order_book_update}")
            ...
            print(f"✅ [Lighter WebSocket] WsClient创建成功: order_book_ids={self._subscribed_markets}")

            # 在单独的线程中运行（因为lighter的WsClient是同步的）
            self._ws_task = asyncio.create_task(self._run_ws_client())
            print(f"✅ [Lighter WebSocket] WsClient任务已启动")
    ...
    def _on_order_book_update(self, market_id: str, order_book: Dict[str, Any]):
        ...
        """
        try:
            print(f"📥 [Lighter WebSocket] _on_order_book_update 被调用: market_id={market_id}, order_book keys={list(order_book.keys()) if isinstance(order_book, dict) else 'N/A'}")
            
            market_index = int(market_id)
            symbol = self._get_symbol_from_market_index(market_index)

            if not symbol:
                print(f"⚠️ [Lighter WebSocket] 未找到market_index={market_index}对应的符号")
                logger.warning(f"未找到market_index={market_index}对应的符号")
                return

            print(f"🔍 [Lighter WebSocket] 解析订单簿: symbol={symbol}, market_index={market_index}")

            # 解析订单簿
            order_book_data = self._parse_order_book(symbol, order_book)
            
            # 🔍 调试：检查订单簿数据（使用INFO级别以便观察）
            if order_book_data and order_book_data.best_bid and order_book_data.best_ask:
                print(f"📥 [Lighter] {symbol} 订单簿更新: bid={order_book_data.best_bid.price}×{order_book_data.best_bid.size}, ask={order_book_data.best_ask.price}×{order_book_data.best_ask.size}, 回调数量={len(self._orderbook_callbacks)}")
                logger.info(f"📥 [Lighter] {symbol} 订单簿更新: bid={order_book_data.best_bid.price}×{order_book_data.best_bid.size}, ask={order_book_data.best_ask.price}×{order_book_data.best_ask.size}, 回调数量={len(self._orderbook_callbacks)}")
            else:
                print(f"⚠️ [Lighter] {symbol} 订单簿数据不完整: best_bid={order_book_data.best_bid if order_book_data else None}, best_ask={order_book_data.best_ask if order_book_data else None}")
                logger.warning(f"⚠️ [Lighter] {symbol} 订单簿数据不完整: best_bid={order_book_data.best_bid if order_book_data else None}, best_ask={order_book_data.best_ask if order_book_data else None}")

            # 缓存
            self._order_books[symbol] = order_book_data

            # 触发回调
            print(f"🔍 [Lighter WebSocket] 准备触发回调，回调数量={len(self._orderbook_callbacks)}")
    ...
    async def _run_direct_ws_subscription(self):
        ...
        while self._running:
            try:
                ...
                # - ping_timeout=None: 禁用超时检测（完全由应用层ping/pong处理）
                # - 主动心跳任务：每30秒主动发送pong，确保连接活跃
                async with websockets.connect(
                    ...
                    # 🔥 发送订单簿订阅（参考 test_sol_orderbook.py）
                    logger.debug("准备发送订单簿订阅")
                    await self._send_orderbook_subscriptions()
    ...
    async def _data_timeout_monitor(self):
        ...
        - 每30秒检查一次最后消息时间
        - 如果超过阈值（120秒）没有收到任何消息，主动断开并触发重连
        - 这样可以及时发现静默断开的情况
        """
        try:
            while self._running:
                try:
                    ...
                    # 检查连接状态
                    if not self._direct_ws or self._direct_ws.closed:
                        logger.debug("⏱️  [数据超时检测] WebSocket已关闭，停止监控")
                        break
                    ...
                    await asyncio.sleep(30)  # 出错后等待30秒再检查
    ...
    async def _handle_direct_ws_message(self, data: Dict[str, Any]):
        ...
        try:
            ...
            # 🔥 处理订单簿更新（参考 test_sol_orderbook.py）
                ...
                try:
                    # 从 channel 中提取 market_index (格式: "order_book:0" 或 "order_book/0")
                    market_index = None
                    if ":" in channel:
                ...
                try:
                    ...
                    if market_index is not None:
                        symbol = self._get_symbol_from_market_index(market_index)
                        if symbol and orderbook_data:
                            # 🔥 应用增量更新到本地订单簿状态（参考 test_sol_orderbook.py）
                            self._apply_orderbook_update(market_index, orderbook_data)
