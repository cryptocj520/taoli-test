...
#
# 🔥 自动路径识别：
#   本脚本会自动识别当前项目路径，并扫描同级目录中的其他账户目录
#
# 源目录：<项目根目录>/config/grid
# 目标目录（自动扫描）：
#   • <项目父目录>/<项目名>_zhanghu1/config/grid
#   • <项目父目录>/<项目名>_zhanghu2/config/grid
#   • <项目父目录>/<项目名>_zhanghu*/config/grid
#   • <项目父目录>/<项目名>_*/config/grid
#
# 示例：
#   如果主项目路径是: /Users/wwgg/Documents/GitHub/crypto-trading
#   则会自动扫描:
#     - /Users/wwgg/Documents/GitHub/crypto-trading_zhanghu1
#     - /Users/wwgg/Documents/GitHub/crypto-trading_zhanghu2
#     - 等等...
#
################################################################################
...
# 主目录配置路径（自动检测）
SOURCE_DIR="$PROJECT_ROOT/config/grid"

# 🔥 自动扫描同级目录，查找其他账户目录
TARGETS=()

# 扫描模式1: crypto-trading_zhanghu* （如: crypto-trading_zhanghu1, crypto-trading_zhanghu2）
for dir in "$PARENT_DIR"/"${PROJECT_NAME}"_zhanghu*/config/grid; do
    if [ -d "$dir" ]; then
        TARGETS+=("$dir")
    fi
done

# 扫描模式2: crypto-trading_* （如: crypto-trading_account1, crypto-trading_test）
for dir in "$PARENT_DIR"/"${PROJECT_NAME}"_*/config/grid; do
    if [ -d "$dir" ]; then
        # 检查是否已添加（避免重复）
        if [[ ! " ${TARGETS[@]} " =~ " $dir " ]]; then
            TARGETS+=("$dir")
...
# 备份目录（自动设置到用户主目录）
BACKUP_DIR="$HOME/.crypto-trading-backups"
...
if [ ${#TARGETS[@]} -eq 0 ]; then
    echo -e "${RED}❌ 错误: 未扫描到任何目标目录${NC}"
    echo -e "${YELLOW}   请确保存在 ${PROJECT_NAME}_zhanghu* 或 ${PROJECT_NAME}_* 目录${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    exit 1
else
if [ ! -z "$TARGET_ACCOUNTS" ]; then
        echo -e "${BLUE}🎯 目标账户:${NC} $TARGET_ACCOUNTS (已过滤)"
else
        echo -e "${BLUE}🎯 目标账户:${NC} 全部 (${#TARGETS[@]}个)"
fi
    
    # 详细显示每个目标目录
    echo -e "${BLUE}📂 目标目录:${NC}"
    for target in "${TARGETS[@]}"; do
        account_name=$(basename $(dirname $(dirname "$target")))
        echo -e "   • $account_name"
    done
fi
